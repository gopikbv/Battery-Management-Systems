...
set_param('Comparison_of_OneRC_Rint_Model','StopTime','3600')

%% Model simulation
sim 'Comparison_of_OneRC_Rint_Model'
...
xlabel('time (s)')
ylabel('Power loss (W)')
legend('Power Loss(Rint)','Power Loss(1RC)')
grid on


figure(5)
plot(ans.R0(:,1),ans.R0(:,2))
hold on
plot(ans.Rin1(:,1),ans.Rin1(:,2))
xlabel('time (s)')
ylabel('Internal resistance (ohms)')
legend('Internal resistance(Rint)','Internal resistance(1RC)')
grid on
...
legend('Polarisation resistance(1RC)')
grid on
...
legend('Terminal Voltage(Rint)','Terminal Voltage(1RC)')
grid on
